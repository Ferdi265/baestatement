...
def fmt_field_line_date(date: Optional[datetime]) -> str:
    if date is None:
        return "     "

    return f"{date:%d.%m}"

def fmt_field_date(date: Optional[datetime]) -> str:
    if date is None:
        return ""
...
def format_cli(stmt: Statement) -> str:
    ...
    for line in stmt.lines:
        ...
        for i, text_line in enumerate(text_lines):
            begin_marker, end_marker = "   " + fmt_field_line_date(None), ""
            if i == 0:
                begin_marker = ">> " + fmt_field_line_date(line.booking_date)
            if i == len(text_lines) - 1:
                end_marker = fmt_field_line_date(line.value_date) + " " + fmt_field_amount(line.amount)

            formatted += f"{begin_marker} {text_line:<57} {end_marker}\n"

    formatted += f">> date: {fmt_field_line_date(stmt.summary.date)}\n"
